...
# CIDR Usage: Invoke-IPMIDump -User Administrator -IP 10.10.11.124/25 -Port 623
# CIDR Usage: Invoke-IPMIDump -User Administrator -IP 10.10.11.124 -Port 623
...
function Test-IP {
    ...
    try {
        ...
        Write-Host "[!] Host $IP does not have IPMI/RMCP+ running or is not vulnerable"
        return
    }
    return $sResponse1
}
...
function Invoke-IPMIDump {
    ...
    foreach ($ip in $ipAddresses){
        Attempt-Retrieve -User $User -Port $Port -IP $ip
    }
}